...
public class DECODETeleop extends LinearOpMode {
    ...
    // 
    final double DRIVETRAIN_SPEED = 0.8;                 // Set the speed of the robot (Must be > 0.8) 
    ...
    final double TICKS_PER_ROTATION = 537.7; // Encoder ticks per motor rotation (adjust for your motor)
    final int INTAKE_ROTATIONS = 1;          // Number of rotations for third ball
    final double INTAKE_ROTATION_POWER = 0.5; // Power for intake rotation
    final double INTAKE_ROTATION_TIMEOUT = 2.0; // Safety timeout for rotation
    ...
    final double LONGEST_SHOOTING_MOTOR_DURATION = 7.0; // Longest time shooting motor can stay on.
    final double SHOOTER_SPOOLUP_TIME = 1.25;    // Wait time to spool up motor
    ...
    private ElapsedTime intakeUpTimer = new ElapsedTime();
    private boolean intakeScoopActive = false;
    private boolean feederUp = false;
    
    // Intake rotation state machine variables
    private ElapsedTime intakeRotationTimer = new ElapsedTime();
    private boolean rotatingIntake = false;
    private int intakeTargetPosition = 0;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            // Gamepad1 fine adjustment via left joystick (turn + forward/backward)
            double fineY = -gamepad1.left_stick_y * 0.15;   // forward/backward
            double fineTurn = gamepad1.left_stick_x * 0.15; // turn/spin
            
            double combinedY = y + fineY;
            double combinedRX = rxRaw + fineTurn;
            
            // Reduce turn power when driving forward/strafing to maintain speed
            double driveMag = Math.min(1.0, Math.hypot(x, combinedY));
            double turnScale = 1.0 - 0.3 * driveMag;  // Tune 0.3 lower for less slowdown, higher for more
            double rx = combinedRX * turnScale;

            double denominator = Math.max(Math.abs(y) + Math.abs(x) + Math.abs(rx), 1.0);
            double leftFrontPower  = (combinedY + x + rx) / denominator;
            double leftBackPower   = (combinedY - x + rx) / denominator;
            double rightFrontPower = (combinedY - x - rx) / denominator;
            double rightBackPower  = (combinedY + x - rx) / denominator;
            ...
            if (gamepad1.dpad_down && !lastDDown) {
                ...
                if (feederUp) {
                    rightBringUp.setPosition(RIGHT_BRINGUP_POS + HALFUP_OFFSET_POS);
                    leftBringUp.setPosition(LEFT_BRINGUP_POS - HALFUP_OFFSET_POS);
                } else {
            ...
            if(HOLD_TO_RUN_INTAKE) // Selecting mode to enable Intake 
            {
                if (gamepad1.left_bumper || gamepad1.right_bumper) {
                    ...
                    // Intake off (only if not rotating)
                    if (!rotatingIntake) {
            ...
            // Apply current state
            if (intakeScoopActive && !rotatingIntake) {
                turnIntakeToScoop();
            } else if (gamepad1.left_trigger > 0.1 && !rotatingIntake) {
                turnIntakeToSpit();
            } else if (!rotatingIntake) {
                intake.setPower(STOP_POWER);
            }
            
            // Third ball intake rotation trigger
            ...
            // Update intake rotation state machine
            updateIntakeRotation();
...
////////////////////// Teleops Loop Ends //////////////////////////////
    ...
    private void turnIntakeToScoop() {
        ...
        //gamepad2.rumbleBlips(1);  // Give rumble feedback to driver
    ...
    private void updateFeedLeft() {
        ...
        if (t > feedingStartSecond && t < (FEEDER_UP_TIME + feedingStartSecond)) {  // Feeding ball
            ...
            leftBringUp.setPosition(REST_POS);
        } else if (t > feedingStartSecond && t < (FEEDER_UP_TIME + FEEDER_FOLLOWUP_DURATION + feedingStartSecond) && !feedingRight) { // extra scope after arm is down
            intake.setDirection(DcMotor.Direction.REVERSE);
            intake.setPower(FULL_POWER);
    ...
    private void updateFeedRight() {
        ...
        if (t > feedingStartSecond && t < (FEEDER_UP_TIME + feedingStartSecond)) { // Feeding ball
            ...
            rightBringUp.setPosition(REST_POS);
        } else if (t > feedingStartSecond && t < (FEEDER_UP_TIME + FEEDER_FOLLOWUP_DURATION + feedingStartSecond) && !feedingLeft) { // extra scope after feeder down
            intake.setDirection(DcMotor.Direction.REVERSE);
            intake.setPower(FULL_POWER);
        } else if (t > feedingStartSecond) { // Feeding completed
            intake.setPower(STOP_POWER);
    ...
    ///////////////////////////////////
    // Start third ball intake rotation
    ///////////////////////////////////
    private void bringThirdBall() {
        if (rotatingIntake) return; // Prevent re-triggering
        
        rotatingIntake = true;
        intakeRotationTimer.reset();
        
        int startPosition = intake.getCurrentPosition();
        intakeTargetPosition = startPosition + (int)(INTAKE_ROTATIONS * TICKS_PER_ROTATION);
        
        intake.setTargetPosition(intakeTargetPosition);
        intake.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        intake.setPower(INTAKE_ROTATION_POWER);
    }
    
    ///////////////////////////////////
    // Update intake rotation state
    ///////////////////////////////////
    private void updateIntakeRotation() {
        if (!rotatingIntake) return;
        
        // Check if reached target or timed out
        if (!intake.isBusy() || intakeRotationTimer.seconds() > INTAKE_ROTATION_TIMEOUT) {
            intake.setPower(0);
            intake.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
